...
//        IntStream.range(0, MAX_BOMBS).mapToObj(i -> this.newBomb()).forEach(Bombs::add); 
    ...
    private void run(GraphicsContext gc) {
        ...
        // Debug print
        //System.out.println("Current redCirclePowerUps size: " + redCirclePowerUps.size());
        ...
        bombs.stream().peek(Rocket::update).peek(Rocket::draw).forEach(e -> {
            ...
            // Check collision with red circle and handle power-up selection if the flag is false
            if (!powerUpMenuTriggered && e.getRedCircle() != null && e.getRedCircle().isActive() && e.getRedCircle().collide(player)) {
